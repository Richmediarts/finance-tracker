...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-foreground">Dashboard</h1><p className="text-muted-foreground">Your financial overview</p></div>
        ...
        {statCards.map((stat) => {
          ...
          return (
            <div key={stat.key} className="bg-card rounded-2xl p-5 border border-border hover:border-primary/30 transition-colors">
              ...
              <p className={`text-2xl font-bold ${stat.key === 'totalIncome' ? 'text-green-400' : stat.key === 'totalExpenses' ? 'text-red-400' : 'text-foreground'}`}>{displayValue}</p>
            </div>
          )
        })}
      </div>
        ...
        <div className="bg-card rounded-2xl p-6 border border-border">
          ...
          {spendingByCategory.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={spendingByCategory} dataKey="amount" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
          ...
          ) : <div className="h-64 flex items-center justify-center text-muted-foreground">No spending data yet</div>}
        </div>
          ...
          <div className="space-y-4">
            {budgetProgress.length > 0 ? budgetProgress.slice(0, 5).map(budget => (
              <div key={budget.categoryName}>
                <div className="flex justify-between text-sm mb-1.5"><span className="font-medium text-foreground">{budget.categoryName}</span><span className={budget.percentUsed > 100 ? 'text-red-400' : 'text-muted-foreground'}>{formatCurrency(budget.spent)} / {formatCurrency(budget.budgeted)}</span></div>
                <div className="w-full bg-muted rounded-full h-2"><div className={`h-2 rounded-full ${budget.percentUsed > 100 ? 'bg-red-500' : 'bg-primary'}`} style={{ width: `${Math.min(budget.percentUsed, 100)}%` }}></div></div>
              </div>
            ...
            {accounts.length > 0 ? accounts.map(account => (
              <div key={account.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="font-medium text-foreground">{account.name}</p><p className="text-sm text-muted-foreground">{account.institution || account.type}</p></div>
                </div>
                <p className={`font-semibold ${account.balance >= 0 ? 'text-green-400' : 'text-red-400'}`}>{formatCurrency(account.balance)}</p>
              </div>
          ...
          <div className="space-y-3">
            {recentTransactions.length > 0 ? recentTransactions.map(tx => (
              <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${tx.category?.color || '#6C5DD3'}20` }}><CreditCard className="w-5 h-5" style={{ color: tx.category?.color || '#6C5DD3' }} /></div>
                  <div><p className="font-medium text-foreground">{tx.description || tx.category?.name || 'Transaction'}</p><p className="text-sm text-muted-foreground">{tx.account.name} • {formatDate(tx.date)}</p></div>
                </div>
                <p className={`font-semibold ${tx.amount >= 0 ? 'text-green-400' : 'text-red-400'}`}>{tx.amount >= 0 ? '+' : ''}{formatCurrency(tx.amount)}</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {goals.slice(0, 3).map(goal => (
              <div key={goal.id} className="p-4 rounded-xl bg-muted/30 border border-border">
                <div className="flex justify-between items-start mb-3"><p className="font-medium text-foreground">{goal.name}</p><span className="text-2xl">{goal.icon || '🎯'}</span></div>
                <div className="w-full bg-muted rounded-full h-2 mb-3"><div className="h-2 rounded-full bg-gradient-to-r from-primary to-purple-400" style={{ width: `${Math.min(goal.progress, 100)}%` }}></div></div>
