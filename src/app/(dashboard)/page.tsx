    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="text-2xl font-bold text-gray-900">{formatCurrency(summary.netWorth)}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <p className="text-sm text-gray-500">Total Expenses (Month)</p>
              ...
              budgetProgress.slice(0, 5).map((budget) => (
                <div key={budget.categoryName}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className={budget.percentUsed > 100 ? 'text-red-600' : 'text-gray-600'}>
                      {formatCurrency(budget.spent)} / {formatCurrency(budget.budgeted)}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${budget.percentUsed > 100 ? 'bg-red-500' : 'bg-blue-500'}`}
            ...
            {accounts.length > 0 ? (
              accounts.map((account) => (
                <div key={account.id} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                  ...
                  <p className={`font-semibold ${account.balance >= 0 ? '' : 'text-red-600'}`}>
                ...
                <div key={tx.id} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{tx.account.name} • {formatDate(tx.date)}</p>
                    </div>
                  </div>
