    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Track your investment portfolio</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className={`text-2xl font-bold flex items-center gap-2 ${totalGainPercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {totalGainPercent >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
            ...
            {investments.map((inv) => (
              <tr key={inv.id} className="hover:bg-gray-50">
                <td className="px-6 py-4">
                  ...
                  {inv.symbol && <p className="text-sm text-gray-500">{inv.symbol}</p>}
                </td>
                ...
                <td className="px-6 py-4 text-right">{formatCurrency(inv.purchasePrice)}</td>
                ...
                <td className={`px-6 py-4 text-right font-medium ${inv.gain >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  <div>{formatCurrency(inv.gain)}</div>
                  <div className="text-xs">({inv.gainPercent.toFixed(2)}%)</div>
                </td>
                <td className="px-6 py-4">
                  <button
